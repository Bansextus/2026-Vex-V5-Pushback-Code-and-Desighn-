...
static Step basic_plan[] = {
    {StepType::DRIVE_MS, kAutonDrivePower, kAutonForwardMs, 0},
    {StepType::TURN_HEADING, 90, 0, 0},
    {StepType::DRIVE_MS, kAutonDrivePower, kAutonForwardMs / 2, 0},
};
...
void draw_brain_ui() {
    ...
    draw_button(gps_btn, "GPS", g_auton_mode == AutonMode::GPS_MODE ? 0x0000FF00 : 0x00FFFFFF);
    draw_button(basic_btn, "BASIC", g_auton_mode == AutonMode::BASIC_MODE ? 0x0000FF00 : 0x00FFFFFF);
    draw_button(run_btn, g_auton_running ? "RUNNING" : "RUN", 0x00FF0000);

    pros::screen::set_pen(pros::c::COLOR_WHITE);
    pros::screen::print(TEXT_MEDIUM, 10, 70, "AUTON: %s",
                        g_auton_mode == AutonMode::GPS_MODE ? "GPS" : "BASIC");
